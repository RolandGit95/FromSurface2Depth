...
dataset = 'regimeB'
folder = f'/home/roland/Projekte/FromSurface2Depth/log/{dataset}/'

name = f'mae_globalmodels.npy'
file = os.path.join(folder, name)
losses = np.load(file)

average = f'{cfg.loss}_average_{cfg.dataset}.npy'
avg_file = os.path.join(folder, average)
avg_reg = np.load(avg_file)

subnetworks = f'{cfg.loss}_subnetworks_{cfg.dataset}.npy'
...
layermodel_losses = np.load('../log/regimeB/mae_layermodels.npy')
